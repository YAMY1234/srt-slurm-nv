...
The default view is **cluster-wide aggregation**: prefill metrics summed
across worker leaders, decode DP-rank metrics scaled up by the inferred
DP factor. This produces one clean line per metric instead of N noisy
per-worker traces, which is the only sensible view for wide-EP / multi-
worker setups.

Usage:
    # Single run (always regenerates):
    python plot_batch_metrics.py outputs/1042857-1p1d-tp4/logs

    # All runs under outputs/ (incremental, skip existing):
    python plot_batch_metrics.py --all

    # All runs under outputs/ (force regenerate):
    python plot_batch_metrics.py --all --force

    # All runs under a custom outputs dir:
    python plot_batch_metrics.py --all --outputs-dir /path/to/outputs

    # With downsample:
    python plot_batch_metrics.py --all --downsample 10

This file is a thin CLI wrapper. The heavy lifting lives in
``srtctl.analysis.batch_metrics`` so the in-flight live snapshotter can
share exactly the same parsing and rendering code.
"""
...
from srtctl.analysis.batch_metrics import process_single_run


def discover_run_dirs(outputs_dir: str) -> list[str]:
    """Find run directories that contain a logs/ subdirectory."""
    run_dirs: list[str] = []
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Plot cluster-wide prefill/decode batch metrics from SGLang worker logs",
...
""",
    ...
    parser.add_argument("--all", "-a", action="store_true", help="Process all run directories under outputs/")
    parser.add_argument(
        "--force",
        "-f",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Downsample factor: keep every Nth bucket when plotting (useful for large logs)",
    )
    ...
    if args.log_dir:
        ...
        ok = process_single_run(log_dir=log_dir, output_path=output_path, downsample=args.downsample)
        if ok:
            print(f"Plot saved to: {output_path}")
        else:
            print(f"Error: No batch data parsed from {log_dir}", file=sys.stderr)
            sys.exit(1)
        return
    ...
    for i, log_dir in enumerate(run_dirs, 1):
        ...
        prefix = f"[{i}/{total}]"
        print(f"{prefix} {run_name} ...", end=" ", flush=True)
