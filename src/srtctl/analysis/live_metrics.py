...
class LiveMetricsSnapshotter:
    ...
    def _tick(self) -> None:
        ...
        except Exception as e:  # noqa: BLE001 - best-effort, never fail the benchmark
...
def try_start_snapshotter(
    ...
    recipe_config: object | None = None,
) -> LiveMetricsSnapshotter | None:
    """Start a snapshotter if the recipe or cluster config opts in, otherwise return ``None``.

    Resolution order:

    1. ``recipe_config`` -- the recipe's ``telemetry.live_metrics`` block
       (a :class:`~srtctl.core.schema.LiveMetricsConfig`). When present it is
       authoritative, so the on/off choice is recorded in the resolved recipe.
    2. Cluster config ``telemetry.live_metrics`` (legacy opt-in) when the
       recipe leaves the block unset.
    ...
    if recipe_config is not None:
        cfg = {
            "enabled": bool(getattr(recipe_config, "enabled", False)),
            "interval_seconds": getattr(recipe_config, "interval_seconds", 60),
            "downsample": getattr(recipe_config, "downsample", 1),
        }
    else:
    ...
    if not cfg or not cfg.get("enabled"):
        logger.info("Live batch-metrics snapshotter disabled (%s)", source)
        return None
